//
//  CreateWalletCommand.swift
//  
//
//  Created by Davide De Rosa on 3/6/21.
//

import Foundation
import JSONRPC

public struct CreateWalletCommand: RPCCommand {
    public struct Response: RPCResponse {
        public let result: WalletResult?
        
        public let error: RPCError?
    }
    
    public let walletName: String
    
    public let disablePrivateKeys: Bool
    
    public let blank: Bool
    
    public init(walletName: String, disablePrivateKeys: Bool = false, blank: Bool = false) {
        self.walletName = walletName
        self.disablePrivateKeys = disablePrivateKeys
        self.blank = blank
    }
}

// MARK: RPCCommand

extension CreateWalletCommand {
    public var method: String {
        return "createwallet"
    }
    
    public var path: String? {
        return nil
    }

    public func argumentsData(withEncoder encoder: JSONEncoder) throws -> [Data] {
        return [
            try encoder.encode(walletName),
            try encoder.encode(disablePrivateKeys)
        ]
    }
    
    public func responseObject(for data: Data, withDecoder decoder: JSONDecoder) throws -> Response {
        return try decoder.decode(Response.self, from: data)
    }
}
